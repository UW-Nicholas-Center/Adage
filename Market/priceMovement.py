    ...
    try:
        ...
        for j in range(0, len(marketCloses)):
            ...
            if(j > 253 and (month == "03" or month=="06" or month=="09" or month=="12") and (day=="28" or day=="29" or day=="30" or day=="31") and resetTimer==0):
                ...
                oneYearVar.append(np.var(marketCloses[j-253:j]))
                sma50.append(np.mean(marketCloses[j-50:j]))
                sma200.append(np.mean(marketCloses[j-200:j]))
                if(j > 1265):
                    fiveYearVar.append(np.var(marketCloses[j-1265:j]))
                else:
                    fiveYearVar.append(0)
            if(resetTimer > 0):
                resetTimer -= 1
    except:
        print("No data for " + ticker[0])

variances = [tickers, dates, oneYearVar, fiveYearVar, sma50, sma200]
...
insertList = '''INSERT INTO variance(ticker, date, oneYearVar, fiveYearVar, sma50, sma200) VALUES(?,?,?,?,?,?)'''
